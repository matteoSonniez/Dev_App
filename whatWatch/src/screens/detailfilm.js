...
const DetailFilm = ({route}) => {
  ...
  const [all_providers, setProviders] = React.useState([]);
  ...
  useEffect(() => {
    ...
    axios({
      ...
      .then(response => {
        setProviders([...all_providers, ...response.data.results.FR.flatrate]);
        console.log(all_providers);
      })
      .catch(error => {
        console.log(error);
      });//https://api.themoviedb.org/3/movie/${id}/watch/providers?api_key=c8ec946078f792a9b9bfd875668c6e5b
  }, []);

  const renderGenre = ({item}) => {
    return (
      <View>
        <TextGenre>{item.name}</TextGenre>
      </View>
    );
  };
  ...
  const renderItem = ({item}) => {
    ...
    const date = item.release_date.slice(0, 4);
    console.log(date);
    return (
      <View>
        <RenderImage 
...
const Decription = styled.View`
  margin-left: 3%;
`;
const SpaceProviders = styled.View`
  margin-right: 3%;
`;
